            ...
            if flags.sum() > 0:
                ...
                for row_idx in flags[flags].index:
                    ...
                    logs.append({
                        ...
                        "gcrit_mode": cfg.gcrit_mode if cfg.outlier_method.startswith("Gcrit") else "",
                        "manual_gcrit": cfg.gcrit if cfg.outlier_method.startswith("Gcrit") else np.nan,
                        "gcrit_alpha": cfg.gcrit_alpha if cfg.outlier_method.startswith("Gcrit") else np.nan,
                        "gcrit_tail": cfg.gcrit_tail if cfg.outlier_method.startswith("Gcrit") else "",
                        "mad_zcrit": cfg.mad_zcrit if cfg.outlier_method.startswith("Robust MAD") else np.nan,
                        "robust_interval_z": cfg.robust_interval_z if cfg.outlier_method.startswith("95% robust interval") else np.nan,
                    })
...
def make_zip_outputs(
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        with pd.ExcelWriter(excel_buf, engine="openpyxl") as writer:
            ...
            pd.DataFrame({
                "setting": ["device_mode", "outlier_method", "max_outliers_per_group", "gcrit_mode", "manual_gcrit", "gcrit_alpha", "gcrit_tail", "mad_zcrit", "robust_interval_z", "bootstrap_ci", "n_boot", "devices", "analytes", "conditions"],
                "value": [
                    cfg.device_mode,
                    cfg.outlier_method,
                    cfg.max_outliers_per_group,
...
Analysis device mode:
  {cfg.device_mode}

Outlier mode:
  {cfg.outlier_method}
Max outliers per condition-device-sample-analyte group:
  {cfg.max_outliers_per_group}
Gcrit mode / alpha / tail:
  {cfg.gcrit_mode} / {cfg.gcrit_alpha} / {cfg.gcrit_tail}
...
st.markdown("""
**Outlier outputs:** raw results are always reported; cleaned results remove flagged outliers only for the selected sensitivity rule.  
**Gcrit automatic mode:** recalculates Grubbs' critical value from the current group size `n`, alpha, and one-/two-sided setting during sequential removal.  
**Robust options:** MAD modified-z and median ± z×MAD-scaled SD are more stable for skewed or non-normal replicate data.
""")
...
- `per_device` = repeatability across selected samples within one device, shown only when device handling is set to analyze each device separately + pooled.
